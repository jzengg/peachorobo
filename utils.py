...
def serialize_pairing(user: DiscordUser, matched_with: DiscordUser) -> SerializedPairing:
...
def deserialize_mystery_dinner(dinner: SerializedMysteryDinner, bot) -> MysteryDinner:
    return MysteryDinner(
        ...
        pairings=[MysteryDinnerPairing(user=bot.get_user(pairing['user']['id']),
                                       matched_with=bot.get_user(pairing['matched_with']['id'])) for pairing in
                  dinner['pairings']])
